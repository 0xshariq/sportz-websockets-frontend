...
export const useMatchData = (): UseMatchData => {
  ...
  const handleMessage = useCallback((message: WSMessage) => {
    if (message.type === 'match_created') setMatches(current => { const match = normalizeMatch(message.data); if (knownIdsRef.current.has(match.id) || current.some(item => item.id === match.id)) return current; knownIdsRef.current.add(match.id); setNewMatchesCount(count => count + 1); return [match, ...current]; });
