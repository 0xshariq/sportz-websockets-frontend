  ...
  return <div className="app-shell font-sans">
    ...
    <div className="dashboard mt-7 grid items-start gap-7 lg:grid-cols-[minmax(0,2fr)_minmax(310px,.95fr)]">
      <main>
        ...
        {!isLoading && !error && matches.length > pageSize && <div className="mt-4 flex items-center justify-between text-xs text-[#666]"><span>Page {effectivePage} of {totalPages}</span><div className="flex gap-2"><button className="rounded-md border-2 border-[#171717] bg-white px-2 py-1 text-[11px] font-extrabold disabled:cursor-default disabled:opacity-40" disabled={effectivePage === 1} onClick={() => setCurrentPage(p => Math.max(1, p - 1))}>Prev</button><button className="rounded-md border-2 border-[#171717] bg-white px-2 py-1 text-[11px] font-extrabold disabled:cursor-default disabled:opacity-40" disabled={effectivePage === totalPages} onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))}>Next</button></div></div>}
      </main>
